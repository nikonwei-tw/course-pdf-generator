...
with col2:
    year = st.text_input("適用入學年度", "115學年度")

st.header("二、學系課程規劃 (可動態新增/刪除列)")
default_courses = pd.DataFrame({
    "科目名稱": ["程式設計", "資料結構", "專題實作(一)", "進階程式設計"],
    "學期或學年": ["一學年", "單學期", "單學期", "單學期"],
    "學分數": [6, 3, 2, 3],
    "必修或必選": ["必修", "必選", "必選", "必選"],
    "擋修": ["無", "無", "無", "程式設計"],
    "模組": ["核心模組", "次要模組一", "次要模組二", "核心模組"]
})
...
uni_courses = pd.DataFrame({
    ...
    "規定說明": ["必修4學分", "必修8學分", "必修10學分(需涵蓋人文藝術(PT)、自然科技(NT)、社會科學(ST)、永續素養(DT)四大領域)", "必修2學分(應修滿4學期)", "必修2學分", "必修4學分", "必修2學分"]
})
